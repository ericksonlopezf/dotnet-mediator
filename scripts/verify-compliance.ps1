...
<#
.SYNOPSIS
    Automated zero-tolerance repository compliance & governance verifier for EricksonLopez.Mediator.
.DESCRIPTION
    Validates:
    ...
    5. Valid GitHub repository links referencing ericksonlopezf/dotnet-mediator.
...
Write-Host "`n[1/8] Checking documentation and template file naming (kebab-case)..." -ForegroundColor Yellow
$docsFiles = Get-ChildItem -Path (Join-Path $RootDirectory "docs") -Recurse -Filter "*.md" -ErrorAction SilentlyContinue
$templateFiles = Get-ChildItem -Path (Join-Path $RootDirectory ".github/ISSUE_TEMPLATE") -Filter "*.md" -ErrorAction SilentlyContinue
$allDocCandidates = @($docsFiles) + @($templateFiles)
$badDocNames = 0
if ($allDocCandidates) {
    foreach ($doc in $allDocCandidates) {
        $filename = $doc.Name
...
if ($badDocNames -eq 0) { Write-Host "  ✅ All documentation and template files use valid kebab-case naming." -ForegroundColor Green }
...
$allCsFiles = Get-ChildItem -Path $RootDirectory -Recurse -Filter "*.cs" | Where-Object {
    $_.FullName -notmatch '[\\/](obj|bin|BenchmarkDotNet\.Artifacts|StrykerOutput|\.git|Generated)[\\/]' -and
...
if ($missingHeaders -eq 0) { Write-Host "  ✅ All C# source files across the solution contain the required MIT copyright header." -ForegroundColor Green }
...
Write-Host "`n[5/8] Checking GitHub identity links (ericksonlopezf/dotnet-mediator)..." -ForegroundColor Yellow
$badLinks = 0
...
foreach ($f in $allTrackedFiles) {
    ...
    for ($i = 0; $i -lt $lines.Count; $i++) {
        if ($lines[$i] -match "github\.com/ericksonlopez/dotnet-mediator\b") {
...
if ($badLinks -eq 0) { Write-Host "  ✅ All GitHub URLs correctly target ericksonlopezf/dotnet-mediator." -ForegroundColor Green }
...
$illegalNoWarn = 0
$projFiles = Get-ChildItem -Path $RootDirectory -Recurse -Include "*.csproj", "*.props" | Where-Object {
    $_.FullName -notmatch '[\\/](bin|obj|BenchmarkDotNet\.Artifacts|StrykerOutput|node_modules)[\\/]'
}
foreach ($proj in $projFiles) {
    $lines = Get-Content $proj.FullName
    for ($i = 0; $i -lt $lines.Count; $i++) {
        if ($lines[$i] -match "<NoWarn>.*(CS0618|CS0619|CA1707|CA1305|CS1591).*</NoWarn>") {
...
# -----------------------------------------------------------------------------
# ImplicitUsings Governance Gate
# -----------------------------------------------------------------------------
Write-Host "`n[Gate: ImplicitUsings] Validating consistent ImplicitUsings configuration..." -ForegroundColor Yellow
$implicitUsingsErrors = 0
$bldPropsPath = Join-Path $targetRoot "Directory.Build.props"
if (Test-Path $bldPropsPath) {
    $bldContent = Get-Content $bldPropsPath -Raw -Encoding utf8
    if ($bldContent -notmatch '<ImplicitUsings>\s*disable\s*</ImplicitUsings>') {
        Write-Host "  ❌ Directory.Build.props must disable ImplicitUsings ('<ImplicitUsings>disable</ImplicitUsings>')." -ForegroundColor Red
        if (Get-Variable -Name "violations" -Scope 0 -ErrorAction SilentlyContinue) { $violations++ }
        $implicitUsingsErrors++
    }
}
$allProjects = Get-ChildItem -Path $targetRoot -Recurse -Filter "*.csproj" | Where-Object {
    $_.FullName -notmatch '[\\/](bin|obj|BenchmarkDotNet\.Artifacts|StrykerOutput|\.git)[\\/]'
}
foreach ($proj in $allProjects) {
    $pContent = Get-Content $proj.FullName -Raw -Encoding utf8
    if ($pContent -match '<ImplicitUsings>\s*enable\s*</ImplicitUsings>') {
        Write-Host "  ❌ Project $($proj.FullName) explicitly enables ImplicitUsings contrary to central policy." -ForegroundColor Red
        if (Get-Variable -Name "violations" -Scope 0 -ErrorAction SilentlyContinue) { $violations++ }
        $implicitUsingsErrors++
    }
}
if ($implicitUsingsErrors -eq 0) {
    Write-Host "  ✅ ImplicitUsings disabled centrally without invalid overrides." -ForegroundColor Green
}

# -----------------------------------------------------------------------------
# Markdown Internal Links Validation Gate
# -----------------------------------------------------------------------------
Write-Host "`n[Gate: Markdown Links] Validating documentation internal links..." -ForegroundColor Yellow
$linkErrors = 0
$mdFiles = Get-ChildItem -Path $targetRoot -Recurse -Filter '*.md' | Where-Object {
    $_.FullName -notmatch '[\\/](bin|obj|BenchmarkDotNet\.Artifacts|StrykerOutput|\.git|node_modules)[\\/]'
}
foreach ($f in $mdFiles) {
    $content = Get-Content $f.FullName -Raw -Encoding utf8
    $matches = [regex]::Matches($content, '\[(?:[^\]]*)\]\((?!https?://|mailto:|#)([^\)#\s]+)(?:#[^\)]*)?\)')
    foreach ($m in $matches) {
        $link = $m.Groups[1].Value
        if ($link -match '\.(md|png|svg|cs|csproj|json|yml|targets|props|snk)$') {
            $resolvedTarget = [System.IO.Path]::GetFullPath((Join-Path (Split-Path $f.FullName) $link))
            if (-not (Test-Path $resolvedTarget)) {
                Write-Host "  ❌ Broken link in $($f.FullName): '$link' (resolved to: '$resolvedTarget')" -ForegroundColor Red
                if (Get-Variable -Name "violations" -Scope 0 -ErrorAction SilentlyContinue) { $violations++ }
                $linkErrors++
            }
        }
...
if ($linkErrors -eq 0) {
    Write-Host "  ✅ All internal documentation and asset links resolve successfully." -ForegroundColor Green
}

# -----------------------------------------------------------------------------
# Version Harmony Gate (Release Please & PackageProjectUrl)
# -----------------------------------------------------------------------------
Write-Host "`n[Gate: Version Harmony] Validating release version harmony (2.0.0)..." -ForegroundColor Yellow
$versionErrors = 0
$expectedVersion = "2.0.0"
$manifestPath = Join-Path $targetRoot ".release-please-manifest.json"
if (Test-Path $manifestPath) {
    try {
        $manifestContent = Get-Content $manifestPath -Raw -Encoding utf8 | ConvertFrom-Json
        if (-not $manifestContent.'.') {
            Write-Host "  ❌ .release-please-manifest.json must contain a valid root version entry." -ForegroundColor Red
            if (Get-Variable -Name "violations" -Scope 0 -ErrorAction SilentlyContinue) { $violations++ }
            $versionErrors++
        }
    } catch {
        Write-Host "  ❌ .release-please-manifest.json is not valid JSON." -ForegroundColor Red
        if (Get-Variable -Name "violations" -Scope 0 -ErrorAction SilentlyContinue) { $violations++ }
        $versionErrors++
    }
}
if (Test-Path $bldPropsPath) {
    $bldContent = Get-Content $bldPropsPath -Raw -Encoding utf8
    if ($bldContent -notmatch "<VersionPrefix>$expectedVersion</VersionPrefix>") {
        Write-Host "  ❌ Directory.Build.props VersionPrefix does not match '$expectedVersion'." -ForegroundColor Red
        if (Get-Variable -Name "violations" -Scope 0 -ErrorAction SilentlyContinue) { $violations++ }
        $versionErrors++
    }
    if ($bldContent -notmatch '<PackageProjectUrl>https://ericksonlopez\.dev/mediator</PackageProjectUrl>') {
        Write-Host "  ❌ Directory.Build.props PackageProjectUrl must be 'https://ericksonlopez.dev/mediator' without 'dotnet-'." -ForegroundColor Red
        if (Get-Variable -Name "violations" -Scope 0 -ErrorAction SilentlyContinue) { $violations++ }
        $versionErrors++
    }
}
$secPath = Join-Path $targetRoot "SECURITY.md"
if (Test-Path $secPath) {
    $secContent = Get-Content $secPath -Raw -Encoding utf8
    if ($secContent -notmatch "2\.0\.x\s*\|\s*:white_check_mark:") {
        Write-Host "  ❌ SECURITY.md does not list 2.0.x as supported." -ForegroundColor Red
        if (Get-Variable -Name "violations" -Scope 0 -ErrorAction SilentlyContinue) { $violations++ }
        $versionErrors++
    }
}
if ($versionErrors -eq 0) {
    Write-Host "  ✅ Complete version harmony verified across props, SECURITY, and release-please configuration." -ForegroundColor Green
}

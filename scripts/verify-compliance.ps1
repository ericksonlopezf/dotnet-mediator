...
if (Test-Path $manifestPath) {
    ...
    if ($manifestContent.'.' -ne $expectedVersion) {
        Write-Host "  ❌ .release-please-manifest.json must be '$expectedVersion' due to release-please." -ForegroundColor Red
        if (Get-Variable -Name "violations" -Scope 0 -ErrorAction SilentlyContinue) { $violations++ }
        $versionErrors++
    }
}
...
if ($versionErrors -eq 0) {
    Write-Host "  ✅ Complete version harmony verified across .release-please-manifest, props, and SECURITY." -ForegroundColor Green
}

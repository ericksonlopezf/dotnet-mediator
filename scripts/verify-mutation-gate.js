...
function loadThresholds(rootDir = process.cwd()) {
  try {
    const generatorConfigPath = path.join(rootDir, 'stryker-generator-config.json');
    if (fs.existsSync(generatorConfigPath)) {
      const config = JSON.parse(fs.readFileSync(generatorConfigPath, 'utf8'));
      const thresholds = config['stryker-config']?.thresholds || config.thresholds || {};
      return {
        high: thresholds.high ?? 100,
        low: thresholds.low ?? 95,
        break: thresholds.break ?? 90,
      };
    }
    ...
    console.warn(`[WARN] Could not load stryker configuration, using defaults (100/95/90): ${err.message}`);
  }
  return { high: 100, low: 95, break: 90 };
}
...
`;
  ...
  console.log(`6. Did it pass the break threshold? : YES (>= ${thresholds.break}%)`);

...
function main() {
  ...
  const pkgName = process.argv[3] || 'Mediator';
  ...
  if (jsonFiles.length > 0) {
    try {
      ...
      if (total === 0) {
        score = 100;
      } else if (total > 0 && (data.mutationScore === undefined || Number.isNaN(Number(data.mutationScore)))) {
  ...
  const passedGate = foundReport && (total === 0 || score >= thresholds.break);
